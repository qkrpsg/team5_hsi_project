					...
					<form class="form-inline" method="post" action="#">
						<div class="form-group pull-left" style="margin-bottom: 1%">
							<a class="btn btn-block btn-primary btn-sm"
								href="<c:url value='/help/qna/Write.pic'/>">문의하기</a>
						</div>
						...
						<c:if test="${not isEmpty}">
							<c:forEach var="item" items="${list}" varStatus="loop">
								<tr>
									<td>${item.Q_INDEX }</td>
									<td id="title"><a
										href="<c:url value='/help/qna/View.pic?q_index=${item.Q_INDEX}'/> ">${item.Q_TITLE }</td>
									...
									<c:if test="${not postempty }">
										<td
											style="background-color: red; color: white; font-weight: bold; width: 100px; text-align: center">답변완료</td>
									</c:if>
